...
public class YangJian : MonoBehaviour
{
    ...
    void doSomething()
    {
        ...
        if(enemys.Count > 0)
        {
            ...
            ani.Play("Attack1");
            GameObject effect = GameObject.Instantiate(skill, transform.position, skill.transform.rotation);
            effect.transform.forward = enemys[0].transform.position-transform.position;
        }
        else
        {
            timer = rate;
